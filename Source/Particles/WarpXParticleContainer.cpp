...
WarpXParticleContainer::DepositCurrent (WarpXParIter& pti,
                                        ...
                                        const amrex::ParticleReal* const var_charge)
{
        ...
        if (WarpX::current_deposition_algo == CurrentDepositionAlgo::Esirkepov) {
            ...
            if        (WarpX::nox == 1){
                doDepositionSharedShapeN<1>(
                        ...
                        bins, box, geom, max_tbox_size, threads_per_block, bin_size, var_charge);
            } else if (WarpX::nox == 2){
            ...
            if (push_type == PushType::Explicit) {
                ...
                if      (WarpX::nox == 1){
                    doEsirkepovDepositionShapeN<1>(
                        ...
                        eb_reduce_particle_shape, EB::enabled(), var_charge );
                } else if (WarpX::nox == 2){
                ...
                if        (WarpX::nox == 1){
                    doChargeConservingDepositionShapeNImplicit<1>(
                        ...
                        WarpX::n_rz_azimuthal_modes, var_charge);
                } else if (WarpX::nox == 2){
            ...
            if (push_type == PushType::Explicit) {
                if        (WarpX::nox == 1){
                    doDepositionShapeN<1>(
                        ...
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, lo, q, WarpX::n_rz_azimuthal_modes, var_charge);
                } else if (WarpX::nox == 2){
...
WarpXParticleContainer::DepositCurrentAndMassMatrices ( WarpXParIter& pti, const RealVector& wp,
                                        ...
                                        amrex::Real dt, [[maybe_unused]] const amrex::ParticleReal* const var_charge )
{
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE((depos_lev==(lev-1)) ||
                                     (depos_lev==(lev  )),
                                     "Deposition buffers only work for lev-1");
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
        ...
        for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
        {
            ...
            DepositCurrent(pti, wp, uxp, uyp, uzp, ion_lev,
                           ...
                           0, np, thread_num, lev, lev, dt, relative_time, PushType::Explicit, var_charge);
        }
#ifdef AMREX_USE_OMP
        }
#endif
    ...
    if (WarpX::do_shared_mem_charge_deposition)
    {
        ...
        if (WarpX::nox == 1){
            doChargeDepositionSharedShapeN<1>(GetPosition, wp.dataPtr()+offset, ion_lev,
                                              ...
                                              WarpX::shared_tilesize, var_charge);
        } else if (WarpX::nox == 2){
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
    {
        ...
        DepositCharge(pti, wp, ion_lev, rho, icomp, 0, np, thread_num, lev, lev, var_charge);
    }
#ifdef AMREX_USE_OMP
    }
#endif
